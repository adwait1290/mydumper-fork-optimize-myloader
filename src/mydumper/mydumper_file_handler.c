...
// OPTIMIZATION: Multiple close_file_threads for parallel fsync
// This provides significant throughput improvement on high-latency storage
// Increased from 4 to 16 for better parallelism with 250K+ files
...
void wait_close_files(){
  if (is_pipe){
    // OPTIMIZATION: Send shutdown signal to all close_file_threads
    guint i;
...
void initialize_file_handler(){
  if (!is_pipe){
    ...
    // OPTIMIZATION: Create multiple close_file_threads for parallel fsync
    // This allows multiple files to be synced concurrently, improving throughput
    // on high-latency storage (NFS, cloud storage, slow SSDs)
    ...
    g_message("Started %d close_file_threads for parallel fsync", NUM_CLOSE_FILE_THREADS);
  }
}
